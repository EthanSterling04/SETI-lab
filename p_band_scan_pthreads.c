...
// Structure to hold the parameters for the worker function
struct WorkerParams {
  int start;
  int end;
  signal* sig;
...
void* worker(void* arg) {
  struct WorkerParams* params = (struct WorkerParams*)arg;

  int start = params->start;
  int end = params->end;
  signal* sig = params->sig;
  int filter_order = params->filter_order;
  double bandwidth = params->bandwidth;
  double* band_power = params->band_power;
  pthread_t thread_id = pthread_self();

  // put ourselves on the desired processor
  cpu_set_t set;
  CPU_ZERO(&set);
  CPU_SET(pthread_getaffinity_np(thread_id, sizeof(set), &set) % num_processors, &set);
  if (sched_setaffinity(0, sizeof(set), &set) < 0) { // do it
    perror("Can't setaffinity"); // hopefully doesn't fail
    exit(-1);
  }
  
  // Now, each thread works on a specific range of bands
  for (int band = start; band < end; band++) {
    ...
    generate_band_pass(sig->Fs,
                       ...
                       filter_order,
                       local_filter_coeffs);
    hamming_window(filter_order, local_filter_coeffs);
...
int analyze_signal(signal* sig, int filter_order, int num_bands, double* lb, double* ub) {
  ...
  struct WorkerParams params[num_threads];

  for (int i = 0; i < num_threads; i++) {
    // Calculate start and end indices for each thread
    int start = i * (num_bands / num_threads);
    int end = (i == num_threads - 1) ? num_bands : (i + 1) * (num_bands / num_threads);

    // Set up parameters for the worker function
    params[i].start = start;
    params[i].end = end;
    params[i].sig = sig;
    params[i].filter_order = filter_order;
    params[i].bandwidth = bandwidth;
    params[i].band_power = band_power;

    // Create thread
    if (pthread_create(&threads[i], NULL, worker, (void*)&params[i]) != 0) {
...
int main(int argc, char* argv[]) {
  ...
  // Added inputs for num_threads and num_processors
  num_threads      = atoi(argv[6]);
  num_processors   = atoi(argv[7]);

  // Cap threads to number of bands
