...
int p_convolve_and_compute_power(int length, double input_signal[],
                                 int order, double coeffs[],
                                 double* power) {

  // Assuming length is a multiple of 8 for simplicity
  int remainder = length % 8;
  int simd_length = length - remainder;

  __m512d sum_vec = _mm512_setzero_pd();

  for (int i = 0; i < simd_length; i += 8) {
    __m512d cur_sum = _mm512_setzero_pd();

    for (int j = order; j >= 0; j--) {
      __m512d input = _mm512_loadu_pd(&input_signal[i - j]);
      __m512d coeff = _mm512_broadcastsd_pd(_mm_load_sd(&coeffs[j]));
      cur_sum = _mm512_fmadd_pd(input, coeff, cur_sum);
    }

    sum_vec = _mm512_fmadd_pd(cur_sum, cur_sum, sum_vec);
  }

  // Sum the remaining elements
  double remaining_sum[8];
  for (int i = simd_length; i < length; i++) {
    ...
    remaining_sum[i - simd_length] = cur_sum;
  }

  // Sum the remaining elements in vectorized form
  __m512d remaining_sum_vec = _mm512_setzero_pd();
  for (int i = 0; i < remainder; i++) {
    __m512d input = _mm512_set1_pd(remaining_sum[i]);
    remaining_sum_vec = _mm512_fmadd_pd(input, input, remaining_sum_vec);
  }

  // Reduce the sum across all elements
  double pow_sum = _mm512_reduce_add_pd(remaining_sum_vec);

  // Reduce the vectorized sum
  pow_sum += _mm512_reduce_add_pd(sum_vec);
