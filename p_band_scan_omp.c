...
int analyze_signal(signal* sig, int filter_order, int num_bands, double* lb, double* ub) {
  ...
  // Set the OMP_PLACES environment variable
  char places_str[256];
  sprintf(places_str, "{0:%d}", num_processors);
  setenv("OMP_PLACES", places_str, 1);

  // Parallelize with reduction
  #pragma omp parallel for num_threads(num_threads) reduction(+:band_power[:num_bands])
  for (int band = 0; band < num_bands; band++) {
    
    double local_band_power;
    ...
    // Accumulate local_band_power to band_power using reduction
    band_power[band] = local_band_power;
  }
